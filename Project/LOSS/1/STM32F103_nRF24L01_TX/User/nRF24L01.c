...
void SPI1_Init(void)
{
	...
	SPI_InitTypeDef   SPI_InitStructure;	// Configure SPI1 pins: SCKand MOSI 
  ...
  GPIO_InitStructure.GPIO_Pin = SD_MOSI|SD_SCK|SD_MISO;
	...
	GPIO_Init(GPIOB, &GPIO_InitStructure);     // Configure PB7 pin: CS pin 
  GPIO_InitStructure.GPIO_Pin = SD_CS;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIO_SetBits(GPIOB,SD_CS);     // SPI1 Config 
  ...
  SPI_InitStructure.SPI_DataSize = SPI_DataSize_8b;
	...
	SPI_Init(SPI1, &SPI_InitStructure);     // SPI1 enable 
  SPI_Cmd(SPI1, ENABLE);*/
...
void nRF24L01_TX_Mode(void)
{
	...
	nRF24L01_Write_Reg(nRF24L01_WRITE_REG+SETUP_RETR,0x13);//设置自动重发间隔时间:500us + 86us;最大自动重发次数:3次
